...
const features = [
  ...
  {
    icon: <ChartPie className="bg-[#657a97] rounded-[12px] w-14 h-10 p-2 m-1" />,
  ...
  {
    icon: <ShieldCheck className="bg-[#657a97] rounded-[12px] w-14 h-10 p-2 m-1" />,
...
function ControleMobile() {
  return (
    <section className="bg-[#F2F4F8] py-16 px-4 sm:px-8 flex flex-col items-center">
      <motion.h1
        initial={{ opacity: 0, y: -20 }}
        whileInView={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6 }}
        viewport={{ once: true }}
        className="max-w-3xl text-3xl font-bold text-center text-black sm:text-5xl lg:text-6xl"
      >
        Contrôlez tout{" "}
      ...
      <motion.p
        ...
        viewport={{ once: true }}
        className="max-w-2xl mt-6 text-base leading-relaxed text-center sm:text-lg lg:text-xl text-black-600"
      >
          ...
          {features.map((feature, idx) => (
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm sm:text-base text-[#717274] mt-2">{feature.description}</p>
              </div>
            </motion.div>
          ))}
        </motion.div>
